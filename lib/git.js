...
export async function ensureAuthenticated() {
  ...
  try {
    ...
    const action = await select({
      ...
      choices: [
        { name: 'GitHub CLI login', value: 'gh' },
        { name: 'Manual (PAT/SSH)', value: 'manual' },
...
export async function checkAndSetupRemote() {
  ...
  console.log(chalk.yellow('\nℹ  No remote found.'));
  const ok  = await confirm({ message: 'Add a GitHub remote now?', default: true });
  if (!ok) { console.log(chalk.red('✖  Remote required.\n')); process.exit(1); }
  const url = await input({ message: 'Remote URL:', validate: i => i.trim() ? true : 'Required' });
